    ...
    <div class="game">
        ...
        <div class="bottom">
          ...
          <div class="btns">
            ...
            <p class="next" @click="next" v-audio>
              <span v-if="step===5"><input id="upload" type="file" accept="image/*" capture="camera" @click="clearFile" @change="getCameraFile">与冰淇淋合影</span>
      ...
      <div class="main poster" v-show="gameOver && postOver">
        ...
        <div class="btns">
          ...
          <p class="next" @click="save"><span>保存图片</span></p>
        </div>
        <!-- 二维码 -->
        <div class="ewm" v-if="ewmVisible">
          <img :src="ewmImg" alt="">
          <p>扫描二维码<br>将图片保存到手机</p>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    createIce(){
      ...
      this.cropperImg.onload = () => {
        ...
        let poxY = this.ctxHeight/2 - 2.3*this.rem*this.pixelRatio
        this.cropperCtx.drawImage(this.cropperImg, -3.46*this.rem*this.pixelRatio, -poxY,this.ctxWidth*this.pixelRatio,this.ctxHeight*this.pixelRatio)
    ...
    posterCanvas(){
      setTimeout(() => {
        ...
        this.iceCreamEle = {
          x: 3.46*this.rem,
          y: (this.ctxHeight/2 - 1.2*this.rem*this.pixelRatio)/this.pixelRatio,
          ...
          centerY:((this.ctxHeight/2 - 1.2*this.rem*this.pixelRatio)/this.pixelRatio)+this.iceCream.height/this.pixelRatio/2
        }
        this.drawIceCanvas()
        Indicator.close()
      },2000)
    ...
    finalCanvas(){
      setTimeout(() => {
        ...
        this.finalImg = finalCanvas.toDataURL('image/jpeg', 0.7)
        Indicator.close()
      },1000)
    },
    save(){
